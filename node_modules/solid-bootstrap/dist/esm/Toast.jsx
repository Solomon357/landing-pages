import { createEffect, createMemo, getOwner, mergeProps, onCleanup, runWithOwner, Show, splitProps, } from "solid-js";
...
const Toast = (p) => {
    ...
    const ToastInner = () => runWithOwner(owner, () => (<div {...props} class={classNames(bsPrefix, local.class, local.bg && `bg-${local.bg}`, !hasAnimation && (local.show ? "show" : "hide"))} role="alert" aria-live="assertive" aria-atomic="true"/>));
    return (<ToastContext.Provider value={toastContext}>
      {() => {
            owner = getOwner();
            return (<Show when={hasAnimation && local.transition} fallback={<ToastInner />}>
